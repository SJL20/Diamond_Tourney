...
`club_teams` is still written, because the year board and Follow-a-team use it for weekends that have no master link. New signups set both `event_teams.team` and `event_teams.club`. The year board prefers `event_teams.team` when it is set (`team:{id}`), then `club_teams`, then the single weekend row. `/admin/teams` is where a site admin edits a master team and removes a year-board profile. Details on a master card loads the private coach email, phone, second contact, and co-owners, then saves them with `PATCH /api/teams/{id}`. The list itself does not include those emails. Removing a year-board profile clears `event_teams.club` and leaves the weekend row. A site admin can delete any master team. That delete takes the club off every weekend, including a weekend that already has a final score, and removes the roster and season book. A director still cannot remove a weekend team that has a final score.
