function isDirector(auth, event) {
  return require(__hooks + "/softball.js").isEventAdmin(event, auth);
}

function canScore(app, event, rec, auth) {
  if (!auth) return false;
  if (isDirector(auth, event)) return true;
...
function postScore(app, event, id, body, auth) {
  ...
  const director = isDirector(auth, event);
  if (body.status) {
...
function botApply(app, body, auth) {
  if (!auth || (auth.get("role") !== "bot" && !isDirector(auth, event))) {
...
function gameDetail(app, event, id, auth) {
  ...
  return {
    ...
    director: isDirector(auth, event),
